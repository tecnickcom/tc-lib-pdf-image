# Contributing to tc-lib-pdf-image

Thank you for your interest in contributing to **tc-lib-pdf-image**.  
...
> **Pull requests are restricted to project collaborators.** If you are not a collaborator, [open an issue](https://github.com/tecnickcom/tc-lib-pdf-image/issues) describing the bug or feature instead.
>
...
```bash
git clone https://github.com/tecnickcom/tc-lib-pdf-image.git
cd tc-lib-pdf-image
make deps
make qa
```
...
This validates the dependencies, checks the formatting, and runs linting, static analysis and the full unit-test suite.
No coverage driver is needed; use `make test-coverage` to produce a coverage report.
...
2. **Search [existing issues](https://github.com/tecnickcom/tc-lib-pdf-image/issues)** to avoid duplicates.

If no existing issue matches, [open a new one](https://github.com/tecnickcom/tc-lib-pdf-image/issues/new) and include:

- A **clear title and description** of the problem.
- The **library version** (`composer show tecnickcom/tc-lib-pdf-image`) and PHP version.
...
> Only project collaborators can open pull requests. If you are not a collaborator, [open an issue](https://github.com/tecnickcom/tc-lib-pdf-image/issues/new) describing the bug (see [Reporting a Bug](#reporting-a-bug)).
...
1. **Open a Feature Request** on [GitHub Issues](https://github.com/tecnickcom/tc-lib-pdf-image/issues/new) describing the use case and the expected behaviour. Keep it at the level of intent and API surface; do not attach an implementation.
...
| Command | Description |
|---------|-------------|
| `make qa` | Validate the dependencies, check the formatting, run linting, static analysis and tests |
...
| `make lint` | Check the coding standards and run the static analysis |
...
./vendor/bin/phpunit test/ImageCacheTest.php
```
...
> Opening pull requests is restricted to project collaborators. If you are an external contributor, please [open an issue](https://github.com/tecnickcom/tc-lib-pdf-image/issues/new) describing the problem or feature in detail instead.
...
If you have a question that is not covered here, feel free to open a [GitHub Discussion](https://github.com/tecnickcom/tc-lib-pdf-image/discussions) or contact the maintainer at [info@tecnick.com](mailto:info@tecnick.com).
